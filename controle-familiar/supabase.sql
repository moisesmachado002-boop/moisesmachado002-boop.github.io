...
create table if not exists public.family_finance_state (
  ...
  loan_people jsonb not null default '{}'::jsonb,
  loan_people_updated_at timestamptz,
  updated_at timestamptz not null default now()
);

-- Compatibilidade com instalações criadas antes das versões 2.4.0, 2.5.0 e 2.8.0.
alter table public.family_finance_state
  ...
  add column if not exists loan_people jsonb not null default '{}'::jsonb,
  add column if not exists loan_people_updated_at timestamptz;
...
-- a sincronização de gastos interpretar que o JSON de gastos mudou.
